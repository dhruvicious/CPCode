...
//     bool operator==(const UUID& other) const noexcept { return this->value ==
//     other.value; }
// };
...
class graphEL {
   ...
   public:
    ...
    void addEdge(const UUID& src, const UUID& dest, double weight = 0.0) {
        if (!vertices.contains(src) || !vertices.contains(dest)) {
            throw std::invalid_argument(
                "One or both the vertices specified do not exist");
        }
    ...
    const std::unordered_map<UUID, Vertex<T>>& getVertices() const noexcept {
        return vertices;
    }
    ...
    void printEdges() const {
        for (auto e : edges) {
            std::cout << "src: " << e.src.str() << " dest: " << e.dest.str()
                      << " weight: " << e.weight << " \n";
        }
    }
...
void run_tests() {
...

    {
        ...
        auto res = g.bfs(b);
        std::cout << "Test 1 (Undirected B->A): "
                  << (res.size() == 2 ? "PASS" : "FAIL") << "\n";
    }

    {
        ...
        auto res = g.bfs(b);
        std::cout << "Test 2 (Directed B ignores A): "
                  << (res.size() == 1 ? "PASS" : "FAIL") << "\n";
    }

    {
        ...
        auto res = g.bfs(v1);
        std::cout << "Test 3 (Cycle Handling): "
                  << (res.size() == 3 ? "PASS" : "FAIL") << "\n";
    }

...
class DynamicArr {
   ...
   public:
    DynamicArr(int n) : _size(0), _capacity(n) {
        if (n < 0) {
            throw std::invalid_argument(
                "Enter a Valid Argument to the Object of the type Dynamic "
                "Array");
        }
    ...
    T& operator[](int index) {
        if (index < 0 || index >= _size) {
            throw std::out_of_range(
                "Index is out of Range for the Object of the Type DynamicArr.");
        }
        return arr[index];
    }
    ...
    void popBack() {
        if (_size == 0) {
            throw std::out_of_range(
                "PopBack called on an empty Object of type DynamicArr");
        }
        _size--;
    }
